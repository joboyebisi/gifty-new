...
// Initialize bot lazily inside the route so it doesn't crash builds if token is missing
...
if (token) {
    ...
    bot.command('pool', async (ctx) => {
        ...
        try {
            ...
            const amountBigInt = BigInt(amount * 1_000_000); // 6 decimals for USDC
            ...
            const claimUrl = `${process.env.FRONTEND_APP_URL || 'http://localhost:3000'}/claim?key=mock_private_key_here`;
...
// In a real Vercel production deployment, you must set the telegram webhook URL:
// https://api.telegram.org/bot<TOKEN>/setWebhook?url=https://<YOUR-VERCEL-DOMAIN>/api/bot
